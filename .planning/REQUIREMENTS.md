...
- [ ] **YIYI-04**: 对话中试衣 BottomSheet 触发（不中断对话流）
- [ ] **YIYI-05**: 快速回复按钮（每条伊伊消息下 2-3 个选项，减少打字）
- [ ] **YIYI-06**: 异常处理（用户放弃 → 温柔收尾 / 都不喜欢 → 引导描述偏好 / LLM 超时 → 规则降级）
...
- [ ] **WKS-02**: 伊伊对话中智能推荐触发（信号检测：预算 luxury/连续 3 次拒绝/特殊场合/说"独一无二"）
- [ ] **WKS-03**: 工作室卡片展示（对话中内联，不打断流程）
- [ ] **WKS-04**: Sprint 手工录入 5-10 家工作室（决策 #25）
...
- [ ] **PHO-01**: 拍照 → FashionSigLIP 编码 → Qdrant 向量检索 → 展示 5 个相似款
...
- [ ] **MINI-01**: 微信小程序 v1（核心功能：伊伊对话+试穿+分享）
