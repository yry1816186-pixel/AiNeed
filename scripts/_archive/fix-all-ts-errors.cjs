...
function fixFile(filePath) {
  ...
  // Skip design-system token files
  ...
  const hasColorsImport = /import\s+[^;]*flatColors\s+as\s+colors/.test(content);
  const hasColorsDestructure = /const\s*\{\s*colors\s*[:}]/.test(content);
  const hasColorsVar = hasColorsImport || hasColorsDestructure;
  ...
  const hasUseStylesDef = /const\s+useStyles\s*=/.test(content);
  ...
  const usesColorsInModuleLevel = (() => {
    ...
    for (const line of lines) {
      ...
      if (inFunction === 0 && /\bcolors\.\w+/.test(line) && !/^\s*\/\//.test(line) && !/^\s*const\s*\{\s*colors/.test(line)) {
        return true;
      }
  ...
  if (usesColors && !hasColorsVar && (usesColorsInStyleSheet || usesColorsInModuleLevel)) {
  ...
  // Fix 2: Add useTheme import + destructure for component-level colors usage
  if (usesColors && !hasColorsVar && !usesColorsInStyleSheet && !usesColorsInModuleLevel) {
    ...
    // Find component function and add destructure after opening brace
    const lines = content.split("\n");
    for (let i = 0; i < lines.length; i++) {
      // Look for function opening that has closing ) followed by {
      const line = lines[i];
      // Match patterns like: }) => { or ): React.FC { or function Foo() {
      if ((line.match(/\)\s*(=>\s*)?\{/) || line.match(/function\s+\w+\s*\([^)]*\)\s*\{/)) &&
          !line.includes("StyleSheet") && !line.includes("import") &&
          (line.match(/^(export\s+)?(function|const)\s+\w+/) || line.match(/^\s*\}\s*\)\s*=>\s*\{/))) {
        // Check next line doesn't already have useTheme
  ...
  // Fix 4: Add missing styles definition (StyleSheet.create stub)
  ...
  // Fix 5: Add missing useStyles definition if createStyles is used
  if (usesUseStyles && !hasUseStylesDef && usesCreateStyles) {
    const lastBrace = content.lastIndexOf("\n}");
    if (lastBrace > 0) {
      content = content.slice(0, lastBrace + 2) +
        `\nconst useStyles = createStyles((colors: any) => ({\n  container: { flex: 1 },\n}));\n` +
        content.slice(lastBrace + 2);
      modified = true;
    }
  }

  // Fix 6: theme variable not defined
