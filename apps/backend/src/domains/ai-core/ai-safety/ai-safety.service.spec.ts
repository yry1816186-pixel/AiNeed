...
describe("AISafetyService", () => {
  ...
  describe("validateResponse", () => {
    ...
    it("应该返回无效结果当检测到幻觉", async () => {
      ...
      const result = await service.validateResponse("今天30度，建议穿羽绒服", { temperature: 30 });
    ...
    it("应该在 ML 服务不可用时使用回退验证", async () => {
      ...
      const result = await service.validateResponse("建议穿短裤参加商务会议", {
        occasion: "business",
      });
    ...
    it("应该在 ML 服务不可用时对温度不一致发出警告", async () => {
      ...
      const result = await service.validateResponse("今天气温5度，建议穿T恤", { temperature: 25 });

      expect(result.issues.some((i) => i.type === "numerical_error")).toBe(true);
    });

    it("应该在 ML 服务返回异常时返回保守的无效结果", async () => {
      mockHttpService.post.mockReturnValue(throwError(() => new Error("Unexpected error")));
  ...
  describe("validateAndCorrect", () => {
    ...
    it("应该在结果有效时保持原样", async () => {
      ...
      const { correctedResponse, validation } = await service.validateAndCorrect(original, {});
