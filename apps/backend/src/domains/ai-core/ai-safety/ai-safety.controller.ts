...
export class AISafetyController {
  ...
  @ApiOperation({
    summary: "验证 LLM 响应是否存在幻觉",
    description: "对 LLM 生成的响应进行幻觉检测，返回验证结果和置信度评分。",
  })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "验证并纠正 LLM 响应",
    description: "对 LLM 响应进行幻觉检测，如检测到幻觉则返回纠正后的内容。",
  })
  ...
  @ApiOperation({
    summary: "快速验证检查",
    description: "轻量级幻觉快速检查，仅返回是否通过验证的布尔结果。",
  })
  @ApiResponse({
    status: 200,
    description: "快速检查结果",
    schema: {
      type: "object",
      properties: { isValid: { type: "boolean", description: "是否通过验证" } },
    },
  })
  @ApiResponse({ status: 400, description: "请求参数错误" })
  @ApiResponse({ status: 401, description: "未授权，需要提供有效的 Access Token" })
  ...
  @ApiOperation({
    summary: "获取幻觉检测统计",
    description: "获取幻觉检测的统计数据，包括检测次数、幻觉率等，支持按日期范围筛选。",
  })
  @ApiQuery({
    name: "startDate",
    required: false,
