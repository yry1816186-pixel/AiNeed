...
export class AIController {
  ...
  @Public()
  @ApiOperation({
    summary: "Body Analysis服务健康检查",
    description: "检查人体数据分析ML服务的连接状态",
  })
  ...
  @ApiOperation({
    summary: "图像分析",
    description: "上传图片进行AI内容分析，支持 JPEG、PNG、WebP 格式，最大 10MB。",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  @ApiOperation({
    summary: "身体分析",
    description: "上传用户照片进行身体数据分析，支持 JPEG、PNG、WebP 格式，最大 10MB。",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  @ApiOperation({
    summary: "相似商品搜索",
    description: "上传图片搜索视觉相似的商品，支持 JPEG、PNG、WebP 格式，最大 10MB。",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  async findSimilar(
    ...
    const results = await this.aiService.findSimilarItemsBuffer(file.buffer, k, category);
  ...
  async findSimilarByItemId(
    ...
    const k = topK ? parseInt(topK, 10) : 10;
    const results = await this.aiService.findSimilarItemsForItem(itemId, k, category);
  ...
  @ApiOperation({
    summary: "穿搭推荐",
    description: "基于指定商品推荐搭配组合，可指定体型、场合等偏好。",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  async recommendOutfit(
    ...
    const results = await this.aiService.recommendOutfit(baseItemId, userBodyType, occasion, topK);
  ...
  @ApiOperation({
    summary: "色彩推荐",
    description: "根据色彩季型（如春、夏、秋、冬）获取推荐颜色。",
  })
  ...
  async getColorRecommendations(
    ...
    const results = await this.aiService.getColorRecommendations(season, category);
  ...
  @ApiOperation({
    summary: "商品AI增强",
    description: "为指定商品添加AI分析结果，包括标签、颜色、风格等属性。",
  })
  @ApiParam({ name: "itemId", description: "商品ID", type: String, format: "uuid" })
  ...
  @ApiOperation({
    summary: "批量商品AI增强",
    description: "批量为商品添加AI分析结果，按限制数量处理。",
  })
  @ApiBody({
    schema: {
      type: "object",
