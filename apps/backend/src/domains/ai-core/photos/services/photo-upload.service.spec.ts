...
describe("PhotoUploadService", () => {
  ...
  const mockStorage = {
    ...
    getFileUrl: jest
      .fn()
      .mockImplementation((path: string) => Promise.resolve(`http://minio:9000/xuno/${path}`)),
  ...
  describe("uploadMultiSize", () => {
    ...
    it("should throw if malware scan fails", async () => {
      mockMalwareScanner.scanImageBuffer.mockResolvedValueOnce({
        safe: false,
        threats: ["malware"],
      });

      await expect(service.uploadMultiSize("user-1", mockFile)).rejects.toThrow(
        BadRequestException
      );
    });
