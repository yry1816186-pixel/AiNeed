...
describe("AiStylistService", () => {
  ...
  >();
  const persistedConfigs = new Map<string, { value: string; updatedAt: Date }>();
  ...
  const mockContextService = {
    ...
    hasBodyProfile: jest
      .fn()
      .mockImplementation(
        (profile?: { bodyType?: string; skinTone?: string; colorSeason?: string } | null) =>
    ...
    extractSlotUpdates: jest.fn().mockImplementation((message: string) => {
      ...
      if (foundStyles.length > 0) {
        updates.preferredStyles = foundStyles;
      }
      if (message.includes("显高") || message.includes("显瘦")) {
        updates.fitGoals = [message.includes("显高") ? "显高" : "显瘦"];
      }
      const budgetMatch = message.match(/(\d+)\s*元?\s*[以到\-~至]\s*(\d+)/);
      if (budgetMatch) {
        updates.budgetMin = Number(budgetMatch[1]);
        updates.budgetMax = Number(budgetMatch[2]);
      }
      const singleBudget = message.match(/预算\s*(\d+)/);
      if (singleBudget && !updates.budgetMax) {
        updates.budgetMax = Number(singleBudget[1]);
      }
      if (message.includes("黑色") || message.includes("白色")) {
        const colors: string[] = [];
        if (message.includes("黑色")) {
          colors.push("黑色");
        }
        if (message.includes("白色")) {
          colors.push("白色");
        }
        if (colors.length > 0) {
    ...
    mergeSlots: jest
      .fn()
      .mockImplementation((current: Record<string, unknown>, updates: Record<string, unknown>) => {
    ...
    processChat: jest
      ...
      .mockImplementation(
        async (
          ...
          buildSessionFn: (uid: string, ctx: unknown) => unknown,
          processMessageFn: (session: unknown, msg: string) => Promise<unknown>
        ) => {
          ...
          for (const historyMessage of (conversationHistory || []).slice(-10)) {
            if (historyMessage?.role && historyMessage?.content) {
              (session as { conversationHistory: unknown[] }).conversationHistory.push({
    ...
    sendMessage: jest
      ...
      .mockImplementation(
        async (
          ...
          if (!session) {
            const record = await mockPrisma.aiStylistSession.findUnique({
              where: { id: sessionId },
            });
            if (record) {
              session =
          ...
          if (!session) {
            throw new Error("AI 造型师会话不存在或已过期");
          }
          return processMessageFn(session, trimmedMessage);
        }
      ),
    uploadSessionPhoto: jest
      .fn()
      .mockImplementation(
        async (
          userId: string,
    ...
    getSessionStatus: jest
      ...
      .mockImplementation(
        async (
          userId: string,
          sessionId: string,
          deriveOrchFn: (s: unknown) => { nextAction: { type: string }; missingFields: string[] },
          buildTemplateFn: (s: unknown, na: unknown) => string,
          buildChatResultFn: (s: unknown, msg: string, opts: Record<string, unknown>) => unknown
        ) => {
          const session = sessionMap.get(`${userId}:${sessionId}`);
          if (!session) {
            throw new Error("AI 造型师会话不存在或已过期");
    ...
    processMessageInSession: jest
      ...
      .mockImplementation(
        async (
          ...
          buildResultFn: (s: unknown, msg: string, opts: Record<string, unknown>) => unknown
        ) => {
          const slotUpdates = mockContextService.extractSlotUpdates(message);
          mockContextService.mergeSlots(
            (session as { state: { slots: Record<string, unknown> } }).state.slots,
            slotUpdates
          );
          (session as { conversationHistory: unknown[] }).conversationHistory.push({
            role: "user",
          ...
          const orchestration = deriveOrchFn(session);
          const { message: assistantMessage, isFallback } = await composeFn(
            session,
  ...
  const mockSessionService = {
    ...
    getSession: jest
      ...
      .mockImplementation(
        async (userId: string, sessionId: string) =>
          sessionMap.get(`${userId}:${sessionId}`) ?? null
      ),
    ...
    persistSession: jest
      .fn()
      .mockImplementation(async (session: { userId: string; id: string }) => {
    ...
    listSessions: jest
      .fn()
      .mockImplementation(async (userId: string, options?: { limit?: number; offset?: number }) => {
  ...
  const mockRecommendationService = {
    ...
    resolveSession: jest.fn().mockResolvedValue({
      ...
      result: {
        outfits: [
          {
            items: [
              { id: "item-top", name: "米色衬衫" },
              { id: "item-bottom", name: "高腰直筒裤" },
            ],
    ...
    submitFeedback: jest
      .fn()
      .mockImplementation(
        (_userId: string, _sessionId: string, _outfitIndex: number, action: string) => ({
          success: true,
          message: action === "like" ? "感谢您的喜欢" : "我们会继续改进",
        })
      ),
  ...
  beforeEach(async () => {
    ...
    mockPrisma.systemConfig.findUnique.mockImplementation(
      async ({ where: { key } }: { where: { key: string } }) => {
        ...
        return record ? { key, value: record.value, updatedAt: record.updatedAt } : null;
      }
    );
    ...
    (service as unknown as { sessionStore: { clear: () => void } }).sessionStore = {
  ...
  it("turns scene and style messages into a photo request when precision is needed", async () => {
    ...
    const styleTurn = await service.sendMessage("user-empty", emptySessionId, "极简");
  ...
  it("restores a persisted session when in-memory cache is empty", async () => {
    ...
    const result = await service.sendMessage("user-123", sessionId, "我要一套面试穿搭");
  ...
  it("resolves an outfit using fallback recommendations when ML items are empty", async () => {
    ...
    expect(mockRecommendationService.resolveSession).toHaveBeenCalled();
  });

  describe("chat 方法测试", () => {
    it("应该处理无会话的聊天请求", async () => {
      const result = await service.chat("user-123", "我要一套面试穿搭", []);
    ...
    it("应该处理包含对话历史的聊天", async () => {
      ...
      const result = await service.chat("user-123", "面试", conversationHistory);
  ...
  describe("submitFeedback 测试", () => {
    ...
    it("应该成功提交不喜欢反馈", async () => {
      ...
      const result = await service.submitFeedback("user-feedback-test-2", sessionId, 0, "dislike");
  ...
  describe("槽位提取测试", () => {
    it("应该从消息中提取场合", async () => {
      ...
      const result = await service.sendMessage("user-slot-test", sessionId, "我要面试穿搭");
    ...
    it("应该从消息中提取风格", async () => {
      ...
      const result = await service.sendMessage("user-slot-test-2", sessionId, "我喜欢极简风格");
    ...
    it("应该从消息中提取预算", async () => {
      ...
      const result = await service.sendMessage("user-slot-test-3", sessionId, "预算500-1000元");
    ...
    it("应该从消息中提取颜色偏好", async () => {
      ...
      const result = await service.sendMessage("user-slot-test-4", sessionId, "我喜欢黑色和白色");
    ...
    it("应该从消息中提取穿搭目标", async () => {
      ...
      const result = await service.sendMessage("user-slot-test-5", sessionId, "我想显高显瘦");
  ...
  describe("边界情况测试", () => {
    it("应该处理不存在的会话", async () => {
      await expect(service.getSessionStatus("user-123", "non-existent-session")).rejects.toThrow();
    });
    ...
    it("应该处理会话过期", async () => {
      ...
      await expect(service.getSessionStatus("user-expire-test", sessionId)).rejects.toThrow();
    });
