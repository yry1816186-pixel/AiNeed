...
export class StockNotificationController {
  constructor(private readonly stockNotificationService: StockNotificationService) {}

  @Post()
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: "Subscribe to stock notification" })
  async subscribe(@Req() req: { user: { id: string } }, @Body() dto: SubscribeDto) {
    return this.stockNotificationService.subscribe(req.user.id, dto.itemId, dto.color, dto.size);
  }
  ...
  async unsubscribe(@Req() req: { user: { id: string } }, @Param("id") id: string) {
