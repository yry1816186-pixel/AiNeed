...
export class GNNCompatibilityService {
  ...
  private nodeMetadata: Map<string, { category: string; attributes: Record<string, string[]> }> =
    new Map();
  ...
  private readonly crossCategoryWeights: Record<
    string,
    {
      embedding: number;
      category: number;
  ...
  > = {
    ...
    "tops-footwear": { embedding: 0.4, category: 0.15, style: 0.3, color: 0.15 },
    "tops-outerwear": { embedding: 0.35, category: 0.15, style: 0.3, color: 0.2 },
    "bottoms-footwear": { embedding: 0.35, category: 0.15, style: 0.3, color: 0.2 },
    "dresses-footwear": { embedding: 0.35, category: 0.15, style: 0.3, color: 0.2 },
    "dresses-accessories": { embedding: 0.4, category: 0.1, style: 0.25, color: 0.25 },
    "accessories-tops": { embedding: 0.4, category: 0.1, style: 0.3, color: 0.2 },
    "accessories-bottoms": { embedding: 0.4, category: 0.1, style: 0.3, color: 0.2 },
  };
  ...
  private async checkServiceHealth(): Promise<void> {
    try {
      ...
      this.logger.warn("Python AI Service not available, using local computation");
  ...
  async addEdge(edge: CompatibilityEdge): Promise<void> {
    ...
    this.logger.debug(`Added edge ${sourceId} <-> ${targetId} with weight ${weight}`);
  }
  ...
  async addHyperedge(hyperedge: Hyperedge): Promise<void> {
    ...
    this.logger.debug(`Added hyperedge ${hyperedge.id} with ${hyperedge.nodeIds.length} nodes`);
  }

  async computeCompatibility(sourceId: string, targetId: string): Promise<CompatibilityResult> {
    this.logger.log(`Computing compatibility between ${sourceId} and ${targetId}`);
    ...
    const embeddingSimilarity = this.cosineSimilarity(sourceFeatures, targetFeatures);
    ...
    const gnnScore = this.computeGNNScore(sourceId, targetId, embeddingSimilarity);
    ...
    const fusedScore = this.fuseScores(gnnScore, hypergraphScore, crossAttentionScore);
  ...
  private computeStyleCompatibility(styles1: string[], styles2: string[]): number {
   ...
   */
  private computeColorCompatibility(colors1: string[], colors2: string[]): number {
    ...
    if (hasNeutral1 || hasNeutral2) {
      // Filter out neutral colors from Lab arrays for CIEDE2000 comparison
      const nonNeutralLab1 = lab1.filter(
        (_, i) => !this.neutralColorNames.has(colors1[i]?.toLowerCase() ?? "")
      );
      const nonNeutralLab2 = lab2.filter(
        (_, i) => !this.neutralColorNames.has(colors2[i]?.toLowerCase() ?? "")
      );

      if (nonNeutralLab1.length === 0 || nonNeutralLab2.length === 0) {
        return 0.85; // Both neutral or one neutral only
      }

      // Check CIEDE2000 between non-neutral colors
      const minDelta = this.minCiede2000(nonNeutralLab1, nonNeutralLab2);
  ...
  private computeGNNScore(sourceId: string, targetId: string, embeddingSimilarity: number): number {
    const neighbors = this.getNeighbors(sourceId);
    const targetNeighbors = this.getNeighbors(targetId);

    const commonNeighbors = neighbors.filter((n) => targetNeighbors.includes(n));
    ...
    return Math.min(1, (embeddingSimilarity + 1) / 2 + neighborBonus + edgeBonus);
  }

  private computeHypergraphScore(sourceId: string, targetId: string): number {
    const sourceHyperedges = this.hypergraphIncidence.get(sourceId) || new Set();
    const targetHyperedges = this.hypergraphIncidence.get(targetId) || new Set();

    const commonHyperedges = [...sourceHyperedges].filter((h) => targetHyperedges.has(h));

    if (commonHyperedges.length === 0) {
      return 0.5;
    }
  ...
  private getCategoryPairKey(cat1?: string, cat2?: string): string | null {
    ...
    if (this.crossCategoryWeights[key]) {
      return key;
    }
    // Try reverse order
    const revKey = `${pair[1]}-${pair[0]}`;
    if (this.crossCategoryWeights[revKey]) {
      return revKey;
    }
    return null;
  }
  ...
  computeOutfitScore(pairwiseScores: number[]): {
    ...
    const variance =
      pairwiseScores.reduce((sum, s) => sum + (s - avgScore) ** 2, 0) / pairwiseScores.length;
  ...
  async batchComputeCompatibility(
    ...
    return Promise.all(
      pairs.map((pair) => this.computeCompatibility(pair.sourceId, pair.targetId))
    );
  }
  ...
  async getRecommendations(
    ...
    for (const [nodeId] of this.nodeFeatures) {
      if (nodeId !== itemId) {
        ...
        if (targetCategories.length > 0 && !targetCategories.includes(nodeMeta?.category || "")) {
          continue;
        }
