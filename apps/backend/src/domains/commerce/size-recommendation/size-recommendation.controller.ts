...
export class SizeRecommendationController {
  constructor(private readonly sizeRecommendationService: SizeRecommendationService) {}

  @Get(":itemId")
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: "Get AI size recommendation" })
  async getRecommendation(@Req() req: { user: { id: string } }, @Param("itemId") itemId: string) {
    const result = await this.sizeRecommendationService.getRecommendation(req.user.id, itemId);
