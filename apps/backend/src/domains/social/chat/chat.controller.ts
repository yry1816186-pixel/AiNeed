...
export class ChatController {
  ...
  async createRoom(@Request() req: RequestWithUser, @Body() dto: CreateChatRoomDto) {
  ...
  @ApiResponse({ status: 401, description: "未授权" })
  async getMyRooms(@Request() req: RequestWithUser, @Query() query: ChatRoomQueryDto) {
  ...
  @ApiResponse({ status: 401, description: "未授权" })
  async getRoomById(@Request() req: RequestWithUser, @Param("id") id: string) {
  ...
  @ApiResponse({ status: 400, description: "请求参数错误" })
  @ApiResponse({ status: 401, description: "未授权" })
  async sendMessage(@Request() req: RequestWithUser, @Body() dto: CreateChatMessageDto) {
  ...
  @ApiResponse({ status: 401, description: "未授权" })
  async getUnreadCount(@Request() req: RequestWithUser, @Param("roomId") roomId: string) {
