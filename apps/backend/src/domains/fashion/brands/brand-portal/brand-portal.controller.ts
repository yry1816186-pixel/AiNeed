...
export class BrandPortalController {
  ...
  async getDashboard(@Request() req: { merchant: { brandId: string } }) {
  ...
  @ApiOperation({
    summary: "更新商品详细数据",
    description: "更新商品的尺码表、材质说明、搭配建议",
  })
  ...
  async updateProductData(
    ...
    return this.brandPortalService.updateProductData(req.merchant.brandId, productId, {
  ...
  async getUserPreferences(@Request() req: { merchant: { brandId: string } }) {
  ...
  async getQRCodeStats(@Request() req: { merchant: { brandId: string } }) {
