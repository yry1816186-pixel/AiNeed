...
export class ClothingDataSourceService {
  ...
  private initializeSources(): void {
    ...
    this.logger.log(
      `Initialized ${this.sources.size} data sources: ${Array.from(this.sources.keys()).join(", ")}`
    );
  }
  ...
  private generateTaobaoSign(params: Record<string, string | number>, secret: string): string {
  ...
  private generateJDSign(params: Record<string, string | number>, secret: string): string {
    const sortedKeys = Object.keys(params).sort();
    const signString = sortedKeys.map((key) => `${key}${params[key]}`).join("");
    return crypto
      .createHash("md5")
  ...
  private normalizeJDItem(item: JDItem): ClothingItem {
    return {
      ...
      images: String(item.imageInfo?.imageList || "")
        ...
        .filter(Boolean),
      mainImage:
        String(item.imageInfo?.whiteImage || item.imageInfo?.imageList || "").split(",")[0] || "",
