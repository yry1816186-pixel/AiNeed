...
export class ForbiddenException extends HttpException {
   ...
   */
  constructor(operation: string, requiredPermission?: string | PermissionType, message?: string) {
   ...
   * 创建非资源所有者异常
   */
  static notOwner(resourceType: string, _resourceId: string): ForbiddenException {
  ...
  static accountDisabled(reason?: string): ForbiddenException {
    const message = reason ? `账户已被禁用: ${reason}` : "账户已被禁用，请联系客服";
  ...
  static ipBanned(): ForbiddenException {
    return new ForbiddenException("check_ip_status", undefined, "您的 IP 已被封禁，请联系客服");
  }

  /**
   * 创建操作频率超限异常
   */
  static rateLimitExceeded(operation: string, retryAfter?: number): ForbiddenException {
   ...
   * 创建资源访问受限异常
   */
  static resourceAccessDenied(resourceType: string, reason?: string): ForbiddenException {
    const message = reason ? `无法访问此${resourceType}: ${reason}` : `无法访问此${resourceType}`;
    return new ForbiddenException(`access_${resourceType.toLowerCase()}`, undefined, message);
  }
