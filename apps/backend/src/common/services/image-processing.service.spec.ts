...
jest.mock("sharp", () => {
  ...
  const createChain = () => {
    const chain = {
      ...
      metadata: jest
        .fn()
        .mockResolvedValue({ width: 200, height: 200, format: "webp", hasAlpha: false }),
    };
...
describe("ImageProcessingService", () => {
  ...
  beforeEach(async () => {
    // Clear accumulated sharp mock chains from previous tests
    jest.requireMock("sharp").default._clearAllChains();
