...
export class ImageProcessingService {
  ...
  async generateSizes(
    ...
    for (const sizeName of sizes) {
      ...
      try {
        ...
        this.logger.error(
          `Failed to generate size '${sizeName}': ${
  ...
  private async generateSize(
    ...
    const webpBuffer = await pipeline.webp({ quality: config.quality, effort: 4 }).toBuffer();
    ...
    return {
      ...
      buffer: webpBuffer,
      width:
        outputMetadata.width ?? (config.name === "original" ? metadata.width ?? 0 : config.width),
      height:
        outputMetadata.height ??
        (config.name === "original" ? metadata.height ?? 0 : config.height),
      format: "webp",
