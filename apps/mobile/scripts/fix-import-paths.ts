...
function main() {
  ...
  for (const file of files) {
    ...
    const designTokensImports = content.match(
      /import\s*\{[^}]*DesignTokens[^}]*\}\s*from\s*["'][^"']*design-tokens["'];?/g
    );
    if (!designTokensImports || designTokensImports.length === 0) {
      continue;
    }
    ...
    for (const oldImport of designTokensImports) {
      if (!oldImport.includes("design-system/theme/tokens/design-tokens")) {
        continue;
      }
      if (oldImport === correctImport) {
        continue;
      }
