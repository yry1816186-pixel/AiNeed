...
export const MagneticButton: React.FC<MagneticButtonProps> = ({
  ...
  return (
    <GestureDetector gesture={gesture}>
      <AnimatedPressable
        style={[style, animatedStyle, enableGlow && glowAnimatedStyle]}
      >
        {children}
      </AnimatedPressable>
    </GestureDetector>
...
export const SwipeAction: React.FC<SwipeActionProps> = ({
  ...
  const leftActionStyle = useAnimatedStyle(() => ({
    ...
    transform: [
      { translateX: interpolate(translateX.value, [-100, 0], [0, -100]) },
    ],
  }));
  ...
  const rightActionStyle = useAnimatedStyle(() => ({
    ...
    transform: [
      { translateX: interpolate(translateX.value, [0, 100], [100, 0]) },
    ],
  }));
...
export const PullToRefresh: React.FC<PullToRefreshProps> = ({
  ...
  return (
    <View style={[styles.pullContainer, style]}>
      ...
      >
        {renderHeader
          ? renderHeader(progress.value)
          : defaultHeader(progress.value)}
      </AnimatedView>
      <GestureDetector gesture={gesture}>
        <AnimatedView style={contentAnimatedStyle}>{children}</AnimatedView>
      </GestureDetector>
