...
interface SharedElementContextValue {
  registerElement: (id: string, ref: React.RefObject<View>, measurements: ElementMeasurements) => void;
  ...
  const gesture = Gesture.Pan()
    .onUpdate((event) => {
      if (enableGestureClose && event.translationY > 0) {
        translateY.value =
          SCREEN_HEIGHT * (1 - snapPoints[0]) + event.translationY;
      }
    })
    .onEnd((event) => {
    ...
    >
      <View style={styles.modalContainer}>
        ...
        <GestureDetector gesture={gesture}>
          <AnimatedView
            style={[styles.modalContent, style, modalAnimatedStyle]}
          >
  ...
  const gesture = Gesture.Pan()
    .onUpdate((event) => {
      animatedPosition.value = Math.max(
        ...
        Math.min(
          snapPointPositions[0] + 50,
          snapPointPositions[initialSnap] + event.translationY,
        ),
      );
    })
    .onEnd((event) => {
  ...
  return (
    ...
    >
      <View style={styles.sheetContainer}>
        ...
        >
          <AnimatedView
            style={[styles.sheetBackdropFill, backdropAnimatedStyle]}
          />
        </TouchableOpacity>
        <GestureDetector gesture={gesture}>
...
export const CrossFadeTransition: React.FC<CrossFadeTransitionProps> = ({
  ...
  return (
    <View style={[styles.crossFadeContainer, style]}>
      <AnimatedView style={[StyleSheet.absoluteFill, fromAnimatedStyle]}>
        {from}
      </AnimatedView>
      <AnimatedView style={[StyleSheet.absoluteFill, toAnimatedStyle]}>
        {to}
      </AnimatedView>
    </View>
  );
...
export const HeroTransition: React.FC<HeroTransitionProps> = ({
  ...
  return (
    <AnimatedView style={[styles.heroContainer, animatedStyle]}>
      {children}
    </AnimatedView>
  );
};
