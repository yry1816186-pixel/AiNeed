...
export function NetworkAware({
  ...
  if (isConnected === null) return <>{children}</>;
...
export function withNetworkAware<P extends object>(
  ...
  ComponentWithNetwork.displayName = `withNetworkAware(${WrappedComponent.displayName || WrappedComponent.name || 'Component'})`;
  return ComponentWithNetwork;
}

const styles = StyleSheet.create({
  wrapper: { flex: 1 },
  banner: { position: 'absolute', top: 0, left: 0, right: 0, zIndex: ZIndex.toast, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: Spacing[2], paddingVertical: Spacing[2], backgroundColor: Colors.warning[500], elevation: 4 },
