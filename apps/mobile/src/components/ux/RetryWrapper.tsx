...
export function RetryWrapper({
  ...
  const execute = useCallback(async () => {
    ...
    try {
      await asyncFn();
      if (mountedRef.current) setState(prev => ({ ...prev, isLoading: false, error: null }));
    } catch (err) {
      if (!mountedRef.current) return;
  ...
  if (state.isLoading) {
    return (
      <View style={[styles.center, style]} accessibilityLabel={accessibilityLabel || '加载中'} accessibilityRole="progressbar">
  ...
  if (state.error) {
    ...
    return (
      <View style={[styles.center, style]} accessibilityLabel={accessibilityLabel || `加载失败，已重试${state.retryCount}次`} accessibilityRole="alert">
        ...
        {state.retryCount < maxRetries && (
          <TouchableOpacity style={styles.retryButton} onPress={handleRetry} activeOpacity={0.8} accessibilityLabel="重试" accessibilityRole="button">
        ...
        <Text style={styles.retryInfo}>已重试 {state.retryCount}/{maxRetries} 次</Text>
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  errorText: { ...Typography.body.md, color: Colors.neutral[600], textAlign: 'center', marginTop: Spacing[4], marginBottom: Spacing[4] },
  retryButton: { flexDirection: 'row', alignItems: 'center', gap: Spacing[2], backgroundColor: Colors.primary[500], paddingHorizontal: Spacing[6], paddingVertical: Spacing[3], borderRadius: BorderRadius.xl, elevation: 3 },
