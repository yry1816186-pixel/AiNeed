...
export function SkeletonScreen({ variant = 'list', count = 4, accessibilityLabel }: SkeletonScreenProps) {
  const label = accessibilityLabel || `加载中${variant === 'list' ? '列表' : variant === 'card' ? '卡片' : '详情'}`;
  return (
...
const styles = StyleSheet.create({
  ...
  listItem: { flexDirection: 'row', gap: Spacing[3], backgroundColor: Colors.neutral.white, padding: Spacing[3], borderRadius: BorderRadius.xl },
  ...
  cardItem: { width: '48%', backgroundColor: Colors.neutral.white, borderRadius: BorderRadius.xl, overflow: 'hidden' },
