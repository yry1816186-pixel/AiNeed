...
export const LiquidGlassCard: React.FC<LiquidGlassCardProps> = ({
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    ...
    shadowOpacity: enableGlow
      ? interpolate(glowValue.value, [0, 1], [0.1, 0.3])
      : 0,
    shadowRadius: enableGlow
      ? interpolate(glowValue.value, [0, 1], [15, 30])
      : 0,
  ...
  const gesture = Gesture.Pan()
    .onUpdate((event) => {
      ...
      translateX.value = Math.max(
        -maxDistance,
        Math.min(maxDistance, event.translationX * 0.3),
      );
      translateY.value = Math.max(
        -maxDistance,
        Math.min(maxDistance, event.translationY * 0.3),
      );
    })
    .onEnd(() => {
      ...
      <GestureDetector gesture={gesture}>
        <AnimatedView style={[animatedStyle, style]}>
          ...
          >
            <LinearGradient
              colors={
                disabled ? ["#9ca3af", "#9ca3af"] : ["#a855f7", "#ec4899"]
              }
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
              style={[
  ...
  return (
    <View style={styles.parallaxContainer}>
      ...
      >
        {headerComponent}
        <AnimatedView
          style={[styles.parallaxHeaderOverlay, headerOpacityStyle]}
        >
          <LinearGradient
...
export const GlowText: React.FC<GlowTextProps> = ({
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    ...
    textShadowRadius: animated
      ? interpolate(glowValue.value, [0, 1], [5, 20])
      : 10,
  }));
...
const ParticleDot: React.FC<ParticleDotProps> = ({ color, size }) => {
  ...
  useEffect(() => {
    y.value = withRepeat(
      withTiming(-50, { duration: fallDuration }),
      -1,
      false,
    );
...
const StaggeredListItem: React.FC<StaggeredListItemProps> = ({
  item,
  index,
  staggerDelay,
}) => {
