...
interface PhotoState {
  ...
  addPhoto: (photo: { id: string; uri: string; type: "body" | "face" | "full" | "outfit" }) => void;
  removePhoto: (id: string) => void;
  setQualityResult: (id: string, result: PhotoQualityResult) => void;
...
export const usePhotoStore = create<PhotoState>((set) => ({
  ...
  addPhoto: (photo) =>
    set((state) => ({
      photos: [
        ...state.photos,
        { ...photo, qualityResult: null, createdAt: new Date().toISOString() },
      ],
    })),
  ...
  setQualityResult: (id, result) =>
    set((state) => ({
      photos: state.photos.map((p) => (p.id === id ? { ...p, qualityResult: result } : p)),
    })),
