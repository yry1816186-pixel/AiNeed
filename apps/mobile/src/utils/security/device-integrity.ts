...
const IOS_JAILBREAK_URL_SCHEMES = ['cydia://', 'undecimus://'] as const;
...
function calculateRiskLevel(status: Omit<DeviceSecurityStatus, 'riskLevel'>): DeviceSecurityStatus['riskLevel'] {
...
async function performFullCheck(): Promise<DeviceSecurityStatus> {
  ...
  if (status.riskLevel !== 'none') {
    reportSecurityEvent({
      type: 'integrity',
      severity: status.riskLevel === 'high' ? 'critical' : status.riskLevel === 'medium' ? 'high' : 'medium',
