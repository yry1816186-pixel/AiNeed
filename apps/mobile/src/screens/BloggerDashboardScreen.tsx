  ...
  return (
    <View style={[styles.chartContainer, { height: chartHeight + 40 }]}>
      ...
      <View style={styles.chartXAxis}>
        {data.filter((_, i) => i === 0 || i === data.length - 1 || i === Math.floor(data.length / 2)).map((point, idx, filtered) => (
...
export const BloggerDashboardScreen: React.FC = () => {
  ...
  const [isBlogger, setIsBlogger] = useState(true); // Would check user's bloggerLevel
  ...
  useEffect(() => {
    fetchTrendData(selectedMetric, period);
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {isLoadingDashboard ? (
          ...
          <View style={styles.metricsGrid}>
            <MetricCard label="浏览量" value={dashboardData.views} change={dashboardData.viewsChange} />
            <MetricCard label="点赞数" value={dashboardData.likes} change={dashboardData.likesChange} />
            <MetricCard label="收藏数" value={dashboardData.bookmarks} change={dashboardData.bookmarksChange} />
            <MetricCard label="评论数" value={dashboardData.comments} change={dashboardData.commentsChange} />
          </View>
        ) : (
            ...
            <View style={styles.enhancedRow}>
              <View style={styles.enhancedCard}>
                <Text style={styles.enhancedValue}>
                  {dashboardData.conversionRate != null ? `${dashboardData.conversionRate.toFixed(1)}%` : '--'}
                </Text>
                <Text style={styles.enhancedLabel}>转化率</Text>
              </View>
              <View style={styles.enhancedCard}>
                <Text style={styles.enhancedValue}>
                  {dashboardData.followerGrowth != null ? `+${dashboardData.followerGrowth}` : '--'}
                </Text>
        ...
        <View style={styles.chartSection}>
          ...
          <View style={styles.metricSelector}>
            {METRIC_OPTIONS.map((opt) => (
              ...
              >
                <Text style={[styles.metricChipText, selectedMetric === opt.key && styles.metricChipTextActive]}>
                  {opt.label}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        {!isBlogger && (
          <View style={styles.upgradeSection}>
            <Ionicons name="rocket-outline" size={32} color="#6C5CE7" />
            <Text style={styles.upgradeTitle}>成为博主解锁更多数据</Text>
            <Text style={styles.upgradeDesc}>
              综合分≥60 且 粉丝≥500 即可自动升级为博主
            </Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  periodTabText: { fontSize: 14, color: theme.colors.textSecondary },
  ...
  noData: { fontSize: 14, color: theme.colors.textTertiary, textAlign: 'center', paddingVertical: 40 },
  enhancedSection: {
    backgroundColor: theme.colors.surface,
    borderRadius: 12,
    padding: 14,
  ...
  enhancedValue: { fontSize: 20, fontWeight: '700', color: '#6C5CE7' },
  enhancedLabel: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 4 },
  chartSection: {
    backgroundColor: theme.colors.surface,
    borderRadius: 12,
    padding: 14,
  ...
  metricChipActive: { backgroundColor: '#F0EDFF' },
  metricChipText: { fontSize: 12, color: theme.colors.textSecondary },
  metricChipTextActive: { color: '#6C5CE7', fontWeight: '600' },
  chartContainer: {
    position: 'relative',
    marginTop: 8,
  },
  ...
  chartXLabel: { fontSize: 10, color: theme.colors.textTertiary },
  chartEmpty: {
    height: 120,
    alignItems: 'center',
    justifyContent: 'center',
  },
  chartEmptyText: { fontSize: 14, color: theme.colors.textTertiary },
  manageBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
