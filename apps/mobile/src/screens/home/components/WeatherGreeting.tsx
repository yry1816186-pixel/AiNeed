...
const WeatherGreeting = memo(
  ({ userName, weatherData, isLoading }: WeatherGreetingProps) => {
    const greeting = useMemo(() => getGreeting(), []);
    const dateStr = useMemo(() => formatDate(), []);
    const weatherIcon = useMemo(() => {
      if (!weatherData?.icon) return 'partly-sunny-outline' as const;
        ...
        {weatherData && (
          <>
            <View style={styles.weatherRow}>
              <Ionicons name={weatherIcon} size={20} color={DesignTokens.colors.brand.terracotta} />
            ...
            {weatherData.suggestion ? (
              <Text style={styles.suggestionText}>
                今天的天气适合{weatherData.suggestion}
              </Text>
            ) : null}
          </>
        )}
