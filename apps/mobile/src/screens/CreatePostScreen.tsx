...
export const CreatePostScreen: React.FC = () => {
      ...
      try {
        ...
        if (draft) {
          ...
          Alert.alert(
            ...
            '检测到未完成的草稿，是否恢复？',
            [
              { text: '放弃', style: 'destructive', onPress: () => AsyncStorage.removeItem(DRAFT_KEY) },
              {
                text: '恢复',
                onPress: () => {
  ...
  const searchItems = useCallback(async (query: string) => {
    ...
    try {
      setSearching(true);
      const response = await apiClient.get<Array<{ id: string; name?: string; mainImage?: string }>>(
        '/clothing',
            ...
            <View style={styles.searchBox}>
              <View style={styles.searchInputRow}>
                ...
                <TextInput
                  ...
                  autoFocus
                />
                <TouchableOpacity onPress={() => { setShowItemSearch(false); setItemSearchQuery(''); setItemSearchResults([]); }}>
...
const styles = StyleSheet.create({
  ...
  tagChipActive: { backgroundColor: '#F0EDFF' },
  tagChipText: { fontSize: 13, color: theme.colors.textSecondary },
  tagChipTextActive: { color: '#6C5CE7', fontWeight: '600' },
