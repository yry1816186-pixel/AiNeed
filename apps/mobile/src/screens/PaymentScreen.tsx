...
export const PaymentScreen: React.FC = () => {
      ...
      try {
        ...
        if (response.success && response.data?.paid) {
          ...
          Alert.alert('支付成功', '您的订单已提交', [
            { text: '查看订单', onPress: () => (navigation as any).navigate('OrderDetail', { orderId }) },
            { text: '返回首页', onPress: () => (navigation as any).navigate('HomeFeed') },
          ]);
        }
      } catch {
  ...
  const handlePayment = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        Alert.alert(
          '支付已发起',
          `请通过${selectedProvider === 'alipay' ? '支付宝' : '微信支付'}完成支付`,
        );
      } else {
  ...
  }, [orderId, selectedProvider, startPolling]);
      ...
      <View style={s.content}>
        ...
        <View style={s.paymentSection}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <View style={[s.providerIcon, { backgroundColor: '#1677FF' }]}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <View style={[s.providerIcon, { backgroundColor: '#07C160' }]}>
...
const s = StyleSheet.create({
  ...
  paymentOption: {
    ...
    paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: theme.colors.divider,
  },
  paymentOptionSelected: { backgroundColor: theme.colors.subtleBg, borderRadius: 10, marginHorizontal: -8, paddingHorizontal: 8 },
  providerIcon: {
    width: 40, height: 40, borderRadius: 10,
    alignItems: 'center', justifyContent: 'center',
  },
  providerIconText: { color: '#fff', fontSize: 18, fontWeight: '700' },
  ...
  pollingBanner: {
    ...
    gap: 8, paddingVertical: 12, marginTop: 16,
    backgroundColor: theme.colors.subtleBg, borderRadius: 10,
  },
  ...
  payButton: {
    ...
    paddingHorizontal: 36, paddingVertical: 14, borderRadius: 24,
  },
