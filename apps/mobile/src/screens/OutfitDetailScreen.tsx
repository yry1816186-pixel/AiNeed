...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Image, Alert, ActivityIndicator, Dimensions, Platform } from 'react-native';
  ...
  const loadOutfit = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const errorMsg = typeof response.error === 'string' ? response.error : response.error?.message || '加载失败';
        setError(errorMsg);
      }
    } catch (err: unknown) {
      setError('网络错误，请重试');
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.errorContainer}>
          ...
          <TouchableOpacity style={styles.retryButton} onPress={loadOutfit} accessibilityLabel="重试加载搭配" accessibilityRole="button">
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()} accessibilityLabel="返回上一页" accessibilityRole="button">
          <Ionicons name='arrow-back' size={24} color={theme.colors.text} />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>搭配详情</Text>
        <TouchableOpacity style={styles.actionButton} onPress={handleToggleFavorite} accessibilityLabel={outfit.isFavorite ? '取消收藏' : '收藏搭配'} accessibilityRole="button">
          <Ionicons name={outfit.isFavorite ? 'heart' : 'heart-outline'} size={24} color={outfit.isFavorite ? theme.colors.error : theme.colors.text} />
        </TouchableOpacity>
      </View>
      <ScrollView style={styles.content}>
        <View style={styles.imageSection}>
          {outfit.thumbnailUri ? <Image source={{ uri: outfit.thumbnailUri }} style={styles.mainImage} accessibilityLabel="搭配图片" /> : <View style={styles.placeholderImage}><Ionicons name='layers-outline' size={80} color={theme.colors.textTertiary} /></View>}
        </View>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.deleteButton} onPress={handleDelete} accessibilityLabel="删除搭配" accessibilityRole="button">
          <Ionicons name='trash-outline' size={20} color={theme.colors.error} />
          <Text style={styles.deleteButtonText}>删除</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.wearButton} accessibilityLabel="标记为今日穿着" accessibilityRole="button">
          <LinearGradient colors={['#4F46E5', '#7C3AED']} style={styles.wearButtonGradient} />
...
const styles = StyleSheet.create({
  ...
  placeholderImage: { width: SCREEN_WIDTH, height: SCREEN_WIDTH * 0.8, backgroundColor: '#F1F3F4', alignItems: 'center', justifyContent: 'center' },
  ...
  wearButton: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 14, borderRadius: 12, overflow: 'hidden' },
