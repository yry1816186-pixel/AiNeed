...
export function navigate<RouteName extends keyof RootStackParamList>(
  ...
  if (checkAndRedirect(name as string)) {
    return;
  }
...
export function navigateTab<RouteName extends keyof MainTabParamList>(
  ...
  if (screen && checkAndRedirect(screen)) {
    return;
  }
...
export function parseDeepLink(url: string): ParsedDeepLink | null {
  ...
  if (!normalizedUrl.startsWith(DEEPLINK_PREFIX)) {
    return null;
  }
...
export function navigateFromPush(data: PushNotificationData, isAuthenticated: boolean): boolean {
  ...
  const screenMap: Record<string, () => void> = {
    ...
    community_post: () =>
      data.targetId && navigateCommunity("PostDetail", { postId: data.targetId }),
